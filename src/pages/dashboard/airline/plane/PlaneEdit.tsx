...
import { AirlinePlaneEdit, AirlineTypeResponse, AxiosErrorResponse } from "@lib/model";
...
const PlaneEdit = () => {
  ...
  const { airline_id, plane_id } = useParams<{ airline_id: string, plane_id: string }>();
  ...
  const [plane, setPlane] = useState<AirlinePlaneEdit>({
    plane_type_id: null,
    name: "",
    description: "",
  });
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: AirlinePlaneEdit) =>
      api
        .put(`/partner/airline/plane/edit/${plane_id}`, data, {
          headers: {
            Authorization: `Bearer ${cookies.token}`,
          },
        })
  ...
  return (
    <>
      ...
      <div className="px-4 py-6 h-dashboard-outlet">
        <FormHead title="Edit Plane" linkBack="../../"/>
